...
generate_completion() {
  ...
  # api-key header (per ell.sh's preflight), not an empty "x-goog-api-key:" --
  # which would also trip ell_curl's "refuse credential over http://" guard,
  # which treats any non-empty auth header as a credential.
  local ELL_CURL_AUTH_HEADER;
  if [ -n "${ELL_API_KEY}" ]; then
    ELL_CURL_AUTH_HEADER="x-goog-api-key: ${ELL_API_KEY}";
